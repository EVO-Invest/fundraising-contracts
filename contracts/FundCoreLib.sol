...
library FundCoreLib {
    ...
    function estimateOwnersProfit(FundMath storage data) view internal returns (uint256) {
        if (data.collected < data.totalInputTokenSalaries + data.allocationsGiven) return 0;
        return data.collected - data.allocationsGiven - data.totalInputTokenSalaries;
    }
    ...
    function closeFundraising(FundMath storage data, uint256 extraFunds, address admin) internal {
        ...
        require(requiredAmountToCloseFundraising(data) == 0, "Can't stop fundraising");
    }
    ...
    function getSalaryAmount(FundMath storage data, address receiver) internal view returns (uint256 salaryAmount) {
        return data.salaries[receiver];
    }
